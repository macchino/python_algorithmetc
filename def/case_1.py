...
hands_dict = {"1": "グー", "2": "パー", "3": "チョキ"}

lose_count = 0
enemy_hand = generator_enemy_hand()
while True:
    my_hand = input("何をだしますか？１：グー、２：パー、３：チョキ")
    if my_hand not in ("1", "2", "3"):
        print("間違った入力です")
        continue
    enemy_hand = next(enemy_hand)
    print(
        "あなたの出した手は：{},相手の出した手は：{}".format(
            hands_dict.get(my_hand), hands_dict.get(enemy_hand)
        )
    )
